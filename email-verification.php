            ...
            <div class="row">
              <div class="col-xs-12 col-sm-8 col-sm-push-2">
                <div class="holder" style="margin: 0;">
                    <div class="mt-side-widget">
                      <header>
                        <h2 style="margin: 0 0 5px;">EMAIL VERIFICATION</h2>
                      </header>
                      <?php
                        require 'includes' . DIRECTORY_SEPARATOR . 'connection' . DIRECTORY_SEPARATOR .  'dbh.inc.php';

                        $email = mysqli_real_escape_string($conn, $_GET['email']);
                        $token = mysqli_real_escape_string($conn, $_GET['validator']);

                        if (!isset($email) || !isset($token)) {
                          echo "Could not validate your request!";
                        } else {
                          if (ctype_xdigit($token) !== false) {
                            $sql = "SELECT * FROM user WHERE user_email = ? AND is_verified = '0'";
                            ...
                            if (!mysqli_stmt_prepare($stmt, $sql)) {
                              ...
                              if (!$row = mysqli_fetch_assoc($result)) {
                                ...
                                $tokenCheck = password_verify($tokenBin, $row['user_token']);

                                if ($tokenCheck === false) {
                                  echo "You need to re-submit your reset request.";
                                  exit();
                                } elseif ($tokenCheck === true) {
                                  $sql = "UPDATE `user` SET `is_verified` = '1' WHERE `user`.`user_email` = ?;";
                                  $stmt = mysqli_stmt_init($conn);
