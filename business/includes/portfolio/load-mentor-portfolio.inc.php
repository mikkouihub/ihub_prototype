...
$sql = "SELECT * FROM `portfolio` WHERE user_id = $mentorID";
...
if (!mysqli_stmt_prepare($stmt, $sql)) {
  ...
  if ($resultCheck > 0) {
    ...
    $sql = "SELECT * FROM `portfolio` WHERE user_id = $mentorID ORDER BY portfolio_ID DESC";
    $stmt = mysqli_stmt_init($conn);
          ...
          '<tr>
            ...
            <td>
              <figure class="port-effect-up">
                <a href="' . 'mentors' . DIRECTORY_SEPARATOR . $url . DIRECTORY_SEPARATOR . 'portfolio' . DIRECTORY_SEPARATOR . $row['portfolio_image'] . '" class="popup-image" data-effect="mfp-3d-unfold">
                  <img style="object-fit: cover; width: 250; height: 250;" class="img-responsive" src="' . 'mentors' . DIRECTORY_SEPARATOR . $url . DIRECTORY_SEPARATOR . 'portfolio' . DIRECTORY_SEPARATOR . $row['portfolio_image'] . '" alt="' . $row['portfolio_name'] . '">
                </a>
              </figure>
            </td>
