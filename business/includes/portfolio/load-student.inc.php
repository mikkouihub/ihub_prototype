  ...
  if ($resultCheck > 0) {
    ...
    if (!mysqli_stmt_prepare($stmt, $sql)) {
      ...
      while ($row = mysqli_fetch_assoc($result)) {
        $fileLocation = 'students' . DIRECTORY_SEPARATOR . $url . DIRECTORY_SEPARATOR . 'portfolio' . DIRECTORY_SEPARATOR . $row['portfolio_image'];
...
// $sql = "SELECT * FROM `student_product_service` WHERE product_service_type = ? AND user_ID = ?";
// $stmt = mysqli_stmt_init($conn);

// if (!mysqli_stmt_prepare($stmt, $sql)) {
//   die('SQL Failed: ' . mysqli_error($conn));
// } else {
//   mysqli_stmt_bind_param($stmt, "ss", $_POST['business_type'], $studentID);
//   mysqli_stmt_execute($stmt);
